...
export async function PUT(request: NextRequest) {
    try {
        ...
        // Parse and validate request body
        const body = await request.json();
        console.log("PUT Education - Request body:", body);

        // Check if ID exists for update
        if (!body.id) {
            return NextResponse.json(
                { error: "ID is required for updating an education entry" },
                { status: 400 }
            );
        }
        ...
        const validation = validateEducationData(body);
        if (!validation.valid) {
            console.error("PUT Education - Validation error:", validation.error);
            return NextResponse.json(
                { error: validation.error },
                { status: 400 }
            );
        ...
        // Forward request to backend
        console.log("PUT Education - Sending to backend:", body);
        const response = await fetch(
            `http://localhost:3000/api/v1/users/education`,
            {
                method: "PUT",
        ...
        if (contentType && contentType.includes("application/json")) {
            ...
            const text = await response.text();
            console.error("PUT Education - Non-JSON response:", text);
            return NextResponse.json(
                { error: "Invalid response from server" },
                { status: 500 }
            );
        ...
        console.log("PUT Education - Backend response:", { status: response.status, data });

        // Handle error response
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || "Failed to update education entry" },
                { status: response.status }
            );
        }
        ...
        console.error("PUT Education - Unhandled error:", error);
        return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        // Parse request body
        const body = await request.json();
        console.log("DELETE Education - Request body:", body);

        // Check if ID exists for deletion
        if (!body.id) {
            return NextResponse.json(
                { error: "ID is required for deleting an education entry" },
                { status: 400 }
            );
        }

        // Forward request to backend
        console.log("DELETE Education - Sending to backend:", body);
        const response = await fetch(
            `http://localhost:3000/api/v1/users/education`,
            {
                method: "DELETE",
        ...
        if (contentType && contentType.includes("application/json")) {
            ...
            const text = await response.text();
            console.error("DELETE Education - Non-JSON response:", text);
            return NextResponse.json(
                { error: "Invalid response from server" },
                { status: 500 }
            );
        ...
        console.log("DELETE Education - Backend response:", { status: response.status, data });

        // Handle error response
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || "Failed to delete education entry" },
                { status: response.status }
            );
        }
        ...
        console.error("DELETE Education - Unhandled error:", error);
        return NextResponse.json(
